...
public class DeptServiceImpl implements IDeptService
{
     ...
     */
    @Override
    public String checkDeptCodeUnique(Dept dept)
    {
        Long deptId = StringUtils.isNull(dept.getDeptId()) ? -1L : dept.getDeptId();
        Dept info = deptMapper.checkDeptCodeUnique(dept.getOrderNum());
        if (StringUtils.isNotNull(info) && info.getDeptId().longValue() != deptId.longValue())
        {
            return UserConstants.DEPT_CODE_NOT_UNIQUE;
        }
        return UserConstants.DEPT_CODE_UNIQUE;
    }
	...
	public List<Dept> selectDeptAll() {
		return selectDeptList(new Dept());
	}
}
